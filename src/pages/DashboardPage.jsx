...
export default function DashboardPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    const [{ data: maq }, { data: corr }, { data: plan }, { data: ejec }, { data: lect }] = await Promise.all([
      supabase.from('maquinas').select('id, estado, deposito_id, hora_inicial, depositos(code)'),
    ...
    const maquinas    = maq  ?? [];
    const correctivos = corr ?? [];
    const planItems   = plan ?? [];
    const ejecutados  = ejec ?? [];
    const lecturas    = lect ?? [];
    ...
    // ranking: máquinas con más correctivos
    const corrPorMaq = {};
    ...
    const ranking = Object.entries(corrPorMaq)
      ...
      .map(([id, count]) => {
        ...
        return { id, count, deposito: m?.depositos?.code ?? '?' };
      });

    // cumplimiento: tareas con al menos 1 ejecución vs total
    ...
    // calcular uso mensual por máquina desde lecturas_horometro
    // para cada máquina: última lectura del mes actual y del mes anterior
    ...
    maquinas.forEach(m => {
      ...
      // agrupar por mes: última lectura de cada mes
      const porMes = {};
      lecsMaq.forEach(l => {
    ...
    setStats({ total, operativas, enTaller, fueraServ, dispPct, corrAbiertas, costoTotal, ranking, cumplimiento, depMap, horasPorDep, horasAnualPorDep, mesActual, anioActual });
    setLoading(false);
  }, []);

  useEffect(() => { fetchStats(); }, [fetchStats]);

  if (loading) return <div style={styles.page}><p style={styles.info}>Cargando...</p></div>;
  ...
  return (
    <div style={styles.page}>
      ...
      {/* Horas de uso por depósito */}
      {Object.keys(stats.horasPorDep).length > 0 && (
        <div style={{ ...styles.section, marginBottom:24 }}>
          <h2 style={styles.sectionTitle}>
            Horas de uso por depósito — {['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'][stats.mesActual-1]} {stats.anioActual}
          </h2>
              ...
              <tbody>
                {stats.ranking.map((r, i) => (
                  <tr key={r.id} style={styles.tr}>
                    ...
                    <td style={{ ...styles.td, fontWeight:700 }}>{r.id.slice(0,8)}…</td>
